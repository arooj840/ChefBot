...
const addGuide = (item, category) => {
  ...
  if (!title || !content) {
    console.warn(`Skipping item in category ${category}: missing title/content`, item);
    return;
  }
  allGuides.push({
    title: title,
...
// ⬇️ PASTE YOUR FRONTEND ARRAYS HERE (from KitchenToolsPage.js) ⬇️
...
const mapCookwareMaterial = (item) => ({
  title: item.name,
  content: item.fullDesc,
  category: 'cookware',
  image: item.image,
...
// Push all
kitchenEssentials.forEach(i => allGuides.push(mapKitchenTool(i)));

knivesData.forEach(i => allGuides.push(mapKnife(i)));
cuttingBoardTypes.forEach(i => allGuides.push(mapBoard(i)));
mixingBowlTypes.forEach(i => allGuides.push(mapBowl(i)));
utensilItems.forEach(i => allGuides.push(mapUtensil(i)));
cookwareTypes.forEach(i => allGuides.push(mapCookwareType(i)));
cookwareMaterials.forEach(i => allGuides.push(mapCookwareMaterial(i)));
crockeryItems.forEach(i => allGuides.push(mapCrockery(i)));
cutleryItems.forEach(i => allGuides.push(mapCutlery(i)));
servingwareItems.forEach(i => allGuides.push(mapServingware(i)));
if (servingCutleryItem) allGuides.push({
  title: servingCutleryItem.name,
  content: servingCutleryItem.fullDesc,
  category: 'servingware',
  image: servingCutleryItem.image,
  video: ''
});

// ==================== 2. MEASURING SKILLS PAGE ====================
// ⬇️ PASTE FROM MeasuringSkillsPage.js ⬇️
const toolsData = [  {
      id: 1,
...
toolsData.forEach(i => allGuides.push(mapMeasuring(i, 'measuring-tools')));
techniquesData.forEach(i => allGuides.push(mapMeasuring(i, 'measuring-techniques')));
estimationData.forEach(i => allGuides.push(mapMeasuring(i, 'estimation')));
conversionData.forEach(i => allGuides.push(mapMeasuring(i, 'conversions')));
precisionData.forEach(i => allGuides.push(mapMeasuring(i, 'precision')));
...
// ⬇️ PASTE FROM PantryBasicsPage.js ⬇️
...
kitchenBasicsData.forEach(i => allGuides.push(mapPantry(i, 'pantry-basics')));
spicesData.forEach(i => allGuides.push(mapPantry(i, 'spices')));
staplesData.forEach(i => allGuides.push(mapPantry(i, 'staples')));
dailyVegetablesData.forEach(i => allGuides.push(mapPantry(i, 'vegetables')));
...
const migrate = async () => {
  try {
    ...
    // ⚠️ DO NOT delete existing guides – only insert new ones
    const toInsert = allGuides.map(g => ({ ...g, createdBy: admin._id }));
    const result = await BeginnersGuide.insertMany(toInsert);
    console.log(`✅ ${result.length} new beginners guides inserted (pages 5-9).`);
    process.exit(0);
  } catch (err) {
